...
#** DictVectorizer(sparse=True)는 sparse=True로 설정하여 데이터를 희소행렬로 출력.
...
print(X_train.shape , y_train.shape)
print(X_test.shape, y_test.shape)
#** 데이터를 일반 행렬로 출력후 표준화 하여 학습 데이터와 테스트 데이터를 만듬
...
#/* solver는 'liblinear', 'newton-cg', 'newton-cholesky', 'sag', 'saga'총 5개가 있고 이중에서 l1이 사용 불가한 solver가 있다.
#/* saga만 l1을 파라미터로 사용 가능하다.
